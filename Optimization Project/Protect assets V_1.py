    ...
    for di in [-1, 0, 1]:
        for dj in [-1, 0, 1]:
            ...
            if ni < 0 or ni >= dim1 or nj < 0 or nj >= dim2:
                var_bars = f"bar_{i}_{j}"
                bars[(i, j)] = pulp.LpVariable(var_bars, cat=pulp.LpBinary)
                continue
...
prob += pulp.lpSum([links[(i, j, k, l)] *(slr - min(region_level[i][j] , region_level[k][l])) for i, j, k, l in links]+ [bars[(i, j)] * (slr -(region_level[i][j]))for (i,j) in bars] )
...
for s in sources:
    for a in assets:
        ...
        if a in paths[s]:
            for ch in paths[s][a]:
                prob += pulp.lpSum([links[(ch[i][0],ch[i][1], ch[i+1][0],ch[i+1][1])] for i in range (len(ch)-1) ] + bars[s]) == 1
...
bars_pos = []
for (i,j) in bars :
    bars_pos=bars_pos + int (bars[(i,j)].varValue) * [(i,j)]

# Add edges to the graph based on the binary variables
for (i, j, k, l), var in links.items():
    if var.value() == 1:
        G.add_edge((i, j), (k, l))
#print('Positions des barrières :', bars_pos, )
# Draw graph of connections
pos = {(i, j): (j, -i) for i in range(dim1) for j in range(dim2)}
nx.draw(B, pos=pos, node_color='lightblue', node_size=40)# ,with_labels=True )
nx.draw(G, pos=pos, node_color='red', node_size=60)# with_labels=True)
nx.draw_networkx_nodes(G, pos=pos, nodelist=assets, node_color='y', node_size=80)
nx.draw_networkx_edges(G,pos=pos , edge_color='g', width=4, )
...
print('Positions des barrières :', bars_pos,G.edges )
nx.draw_networkx_edges(G, pos=pos, edgelist=[], edge_color='red', width=40)
plt.title("Graph of barriers")

for (i,j) in bars_pos :
    x,y= j,-i
    if i == dim1-1 :plt.plot([x-0.28, x+0.28], [y-0.28, y-0.28], color='g', linewidth=4) # ligne horizontale supérieure
    if i==0 : plt.plot([x-0.28, x+0.28], [y+0.28, y+0.28], color='g', linewidth=4) # ligne horizontale inférieure
    if j ==0: plt.plot([x-0.28, x-0.28], [y-0.28, y+0.28], color='g', linewidth=4) # ligne verticale gauche
    if j ==dim2-1: plt.plot([x+0.28, x+0.28], [y-0.28, y+0.28], color='g', linewidth=4) # ligne verticale droite
